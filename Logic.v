...
(*
if ~(P /\ ~P) = True then P /\ ~P = true then P = true /\ P = false contra.
    ...
    + intros H.
      ...
      * apply IH.
      (*intros!!!!*)
        ...
        apply H. simpl.
        right. apply H1.
    + intros [H1 H2] x0 [H3 | H3].
      * rewrite <- H3.
        apply H1.
      * destruct IH as [_ IH].
        apply IH with (x:= x0) in H2.
        -- apply H2.
        -- apply H3.
Qed.
(** [] *)
...
Proof.
  intros l H.
  (*specialize (in_not_nil nat 41 l) as H1.*)
...
Proof.
  intros x y.
  specialize not_true_iff_false with (b := (x =? y)) as H.
  rewrite <- H.
  split.
  - unfold not.
    intros H1 H2.
...
Proof.
  ...
  - induction l as [| x l' IH].
    + simpl. reflexivity.
    + simpl. destruct (test x).
      * intros [_ H2]. simpl. apply IH. apply H2.
...
(*stop immediately when meet a (x := false)
