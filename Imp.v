...
Proof.
  remember optimize_0plus_sound as H.
  intros b.
  induction b;
  try reflexivity;
  try (simpl; repeat rewrite H; reflexivity);
  try simpl.
...
Proof.
  ...
  - (* -> *)
    ...
    + (* E_AMult *)
      rewrite IHaevalR1.  rewrite IHaevalR2.  reflexivity.
  - (* <- *)
    generalize dependent n. (*对a归纳，不要把n引入*) 
    induction a;
       simpl; intros; subst.
    + (* ANum *)
      apply E_ANum.
    + (* APlus *)
      apply E_APlus.
      * apply IHa1. reflexivity. (*如果不generalize dependent n，这里就无法把n替换成aveal a1*)
...
Inductive no_whilesR: com -> Prop :=
  ...
  | No_Whiles_Seq : forall c1 c2, (no_whilesR <{ c1 }>) -> 
                                  (no_whilesR <{ c2 }>) ->
                                  (no_whilesR <{ c1;c2}> )
  | No_Whiles_If : forall b c1 c2, (no_whilesR <{ c1 }>) -> 
                                    (no_whilesR <{ c2 }>) ->
...
Proof.
  ...
  - induction c;
    ...
    intros H;
    try constructor;
    try (try apply IHc1; try apply IHc2; apply andb_true_iff in H;
